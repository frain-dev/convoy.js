...
import { ConvoyDefaultError } from "../models/errors/convoy-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ConvoyDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | ConvoyDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ConvoyDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ConvoyDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
