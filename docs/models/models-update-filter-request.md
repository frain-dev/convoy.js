# ModelsUpdateFilterRequest

## Example Usage

```typescript
import { ModelsUpdateFilterRequest } from "convoy.js/models";

let value: ModelsUpdateFilterRequest = {};
```

## Fields

| Field                                                          | Type                                                           | Required                                                       | Description                                                    |
| -------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------- |
| `body`                                                         | Record<string, *any*>                                          | :heavy_minus_sign:                                             | Body matching criteria (optional)                              |
| `enabledAt`                                                    | [models.ModelsOptionalTime](../models/models-optional-time.md) | :heavy_minus_sign:                                             | Non-null when this filter is active.                           |
| `eventType`                                                    | *string*                                                       | :heavy_minus_sign:                                             | Type of event this filter applies to (optional)                |
| `headers`                                                      | Record<string, *any*>                                          | :heavy_minus_sign:                                             | Header matching criteria (optional)                            |
| `isFlattened`                                                  | *boolean*                                                      | :heavy_minus_sign:                                             | Whether the filter uses flattened JSON paths (optional)        |
| `path`                                                         | Record<string, *any*>                                          | :heavy_minus_sign:                                             | Path matching criteria (optional)                              |
| `query`                                                        | Record<string, *any*>                                          | :heavy_minus_sign:                                             | Query matching criteria (optional)                             |